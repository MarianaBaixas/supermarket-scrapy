...
class BramilPetropolisSpider(scrapy.Spider):
    name = "bramil_petropolis"
    start_urls = ["https://www.bramilemcasa.com.br/"]

    def parse(self, response):
        for elemento in range(len(categorias)):
            for pagina in range(1, 4):
                url = f"https://api-loja.bramilemcasa.com.br/v1/loja/classificacoes_mercadologicas/departamentos/{categorias[elemento]}/produtos/filial/1/centro_distribuicao/16/em_oferta?page={pagina}"
                headers = {
    ...
    def parse_api(self, response):
        ...
        for product in data['data']:
            if product["disponivel"] == True:
                ...
                product_item['mercado'] = 'Bramil Petrópolis'
